...
変更ファイルのパスに基づいてリスクを分類:

| 変更パス | リスクカテゴリ | 重要度 |
|---------|-------------|--------|
| `supabase/migrations/` | DB マイグレーション | High |
| `middleware.ts` | 認証フロー | High |
| RLS ポリシーを含む SQL | データアクセス | High |
| `packages/types/` | API 契約 | Medium |
| `apps/web/app/api/` | API エンドポイント | Medium |
| `apps/web/components/` | UI コンポーネント | Low |
...
`.claude/reviewer-profile.md` を読み込み、レポートの表現を最適化する。
