...
このスキルはプロジェクトのテスト環境を自動検出します。以下のファイルを確認してテストフレームワークとコマンドを特定してください：

1. **CLAUDE.md** --- プロジェクト固有のテストコマンドが記載されている場合はそれを使用
2. **package.json** --- `scripts.test` からテストコマンドを検出（npm/pnpm/yarn）
3. **Makefile** --- `test` / `check` ターゲットを検出
4. **pyproject.toml / setup.cfg** --- pytest 等の設定を検出
5. **Cargo.toml** --- `cargo test` を使用
6. 既存のテストファイルパターンから推定（`*.test.ts`, `*_test.py`, `*_test.go` 等）
...
## コマンド
プロジェクトから検出したテストコマンドを使用してテストファイルを実行する。
...
Agent ツールで test-designer エージェント（Plan, opus）を起動し、テストケースを設計する。

### Step 2: 並列でテスト実装 & 既存テスト確認

2つの Agent ツール呼び出しを並列で実行：
- テスト実装エージェント（general-purpose, sonnet）
- 既存テスト確認エージェント（general-purpose, haiku）

### Step 3: Red - テストが失敗することを確認

プロジェクトのテストコマンドで新しいテストを実行し、失敗を確認（まだ実装がないため）。

### Step 4: Green - テストを通す

Agent ツールで feature-implementer エージェント（general-purpose, sonnet）を起動し、テストを通す最小限のコードを実装。

### Step 5: テスト再実行 & 確認

テストコマンドで全テストが通ることを確認。
