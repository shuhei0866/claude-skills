...
ワークツリーの分岐元（ベースブランチ）を決定する：

- **引数でベースブランチが指定されている場合**: そのブランチを使用
- **RDD の `release/*` ブランチ内でサブタスク分解する場合**: リリースブランチがベース
- **それ以外**: main（デフォルト）

```bash
# ベースブランチの決定
# 引数で指定: /git-worktrees feature/chat/api --base release/chat-feature
# デフォルト: main
...
ワークツリー作成前にベースブランチの最新状態を取得する。ローカルが古いまま分岐すると、存在するはずのコードが見つからない空振りが発生する。

```bash
# リモートに存在すれば fetch（ローカル専用ブランチの場合はスキップ）
git fetch origin "$BASE_BRANCH" 2>/dev/null
```

- リモートに存在するブランチ → `origin/$BASE_BRANCH` から分岐（常に最新）
- ローカル専用ブランチ（未 push の `release/*` 等） → ローカルの `$BASE_BRANCH` から分岐
...
[Run pnpm install]
[Run pnpm test:run - 459 passing]

Worktree ready at /project/.worktrees/chat-feature
Tests passing (459 tests, 0 failures)
Ready to implement chat-feature release
```

### サブタスク用ワークツリー（リリースブランチがベース）
